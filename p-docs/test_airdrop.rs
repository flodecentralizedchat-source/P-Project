...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Airdrop status: {} total, {} distributed, {} recipients, {} claimed", 
             status.total_amount, status.distributed_amount, status.total_recipients, status.claimed_recipients);
    ...
    println!("Created time-limited airdrop. Active: {}", timed_airdrop.is_active());
    
    // Test Merkle tree
    let leaves = vec!["user1".to_string(), "user2".to_string(), "user3".to_string()];
    ...
    // Test batch claiming
    let batch_users = vec!["user2".to_string(), "user3".to_string()];
    match airdrop.batch_claim(batch_users) {
        Ok(claimed_amounts) => {
            println!("Batch claimed {} users", claimed_amounts.len());
            for (user, amount) in claimed_amounts {
                println!("  {} claimed {} tokens", user, amount);
            }
        }
        Err(e) => println!("Failed to batch claim: {}", e),
    }
    
    // Test category functionality
    let mut category_airdrop = AirdropContract::new(total_amount);
    let category_recipients = vec![
        ("user4".to_string(), 100.0),
        ("user5".to_string(), 200.0),
    ];
    
    category_airdrop.add_recipients_with_category(category_recipients, Some("early_supporter".to_string())).unwrap();
    println!("Added recipients with category");
    
    if let Some(category) = category_airdrop.get_user_category("user4") {
        println!("User4 category: {}", category);
    }
