...
fn test_full_tokenomics_implementation() {
    ...
    vesting_contract.create_team_vesting(team_member.clone(), team_amount, start_date).unwrap();
    ...
    vesting_contract.create_advisor_vesting(advisor.clone(), advisor_amount, start_date).unwrap();
    ...
    staking_contract.stake_tokens("staker1".to_string(), 100000.0, 365).unwrap(); // 1 year
    staking_contract.stake_tokens("staker2".to_string(), 200000.0, 730).unwrap(); // 2 years
    ...
    treasury.allocate_funds("Ecosystem Grants".to_string(), 200000.0, "Funding ecosystem development".to_string()).unwrap();
    treasury.allocate_funds("Charity Endowment".to_string(), 200000.0, "Charitable initiatives".to_string()).unwrap();
    treasury.allocate_funds("Treasury Reserve".to_string(), 500000.0, "General treasury reserve".to_string()).unwrap();
    ...
    token.add_liquidity("pool1".to_string(), "liquidity", 17500000.0).unwrap();
    
    // Lock liquidity for 24 months
    token.lock_liquidity("pool1".to_string(), 17500000.0).unwrap();
    ...
    println!("User1 activity before high activity transfers: {}", user1_activity_before);
    
    // Simulate high activity for a user (more than 10 transactions to trigger increased burn rate)
    for _ in 0..20 {  // Increase to 20 transactions to ensure we exceed the threshold
    ...
    println!("User1 activity after high activity transfers: {}", user1_activity_after);
    ...
    let high_activity_burn = supply_before_high_activity_transfer - supply_after_high_activity_transfer;
    ...
    token.transfer("liquidity", "partner_platform_1", 500000.0).unwrap();
    token.transfer("liquidity", "partner_platform_2", 300000.0).unwrap();
    ...
    println!("Partner 1 balance: {}, Partner 2 balance: {}", partner1_balance, partner2_balance);
    ...
    staking_contract.stake_tokens("service_provider".to_string(), 5000.0, 180).unwrap();
    
    // Verify service provider has staked tokens
    let provider_staking = staking_contract.get_staking_info("service_provider").unwrap();
