...
contract PProjectToken {
     ...
     * @param spenderAddress Address that is allowed to spend tokens
    ...
    function _approve(address ownerAddress, address spenderAddress, uint256 amount) internal {
        require(ownerAddress != address(0), "Approve from zero address");
        require(spenderAddress != address(0), "Approve to zero address");
     ...
     */
    function setBotCooldownPeriod(uint256 cooldownSeconds) external onlyOwner {
        botCooldownPeriod = cooldownSeconds;
    }
    ...
    function setUniswapRouter(address newAddress) external onlyOwner {
        ...
        // For testing purposes, we'll set the pair directly instead of trying to get/create it
    ...
    /**
     * @dev Set the Uniswap V2 pair address directly (for testing)
