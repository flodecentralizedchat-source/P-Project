...
mod tests {
    ...
    fn test_create_collection() {
        ...
        let collection_id = nft_contract
            .create_collection(
    ...
    fn test_transfer_nft() {
        ...
        // Mint NFT
        let nft_id = nft_contract
            .mint_nft(collection_id, "owner1".to_string(), metadata, 5.0)
            .unwrap();

        // Transfer NFT
        let result =
            nft_contract.transfer_nft(nft_id.clone(), "owner1".to_string(), "owner2".to_string());
        assert!(result.is_ok());
    ...
    fn test_withdraw_earnings() {
        ...
        nft_contract
            .owner_balances
            .insert("user1".to_string(), 150.0);

        // Withdraw earnings
        let remaining_balance = nft_contract
            .withdraw_earnings("user1".to_string(), 50.0)
            .unwrap();
    ...
    fn test_get_user_nfts() {
        ...
        // Mint NFTs
        nft_contract
            .mint_nft(collection_id.clone(), "owner1".to_string(), metadata1, 5.0)
            .unwrap();

        nft_contract
            .mint_nft(collection_id, "owner1".to_string(), metadata2, 5.0)
            .unwrap();
    ...
    fn test_collection_supply_limit() {
        ...
        let result2 = nft_contract.mint_nft(collection_id, "owner2".to_string(), metadata, 5.0);
        assert!(result2.is_err());
        assert_eq!(
            result2.unwrap_err(),
            "Collection has reached maximum supply"
        );
    }
