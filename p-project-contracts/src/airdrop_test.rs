...
fn test_airdrop_recipient_management() {
    ...
    let recipients = vec![("user1".to_string(), 1000.0), ("user2".to_string(), 2000.0)];
...
fn test_airdrop_with_categories() {
    ...
    let result = airdrop_contract
        .add_recipients_with_category(recipients, Some("early_supporter".to_string()));
    assert!(result.is_ok());

    // Check that categories are set
    assert_eq!(
        airdrop_contract.get_user_category("user1"),
        Some(&"early_supporter".to_string())
    );
    assert_eq!(
        airdrop_contract.get_user_category("user2"),
        Some(&"early_supporter".to_string())
    );
}
