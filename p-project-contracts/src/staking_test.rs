...
mod tests {
    ...
    fn test_staking_contract_creation() {
        ...
        assert!(!staking.emergency_withdrawals_enabled);
    }
    ...
    fn test_unstake_tokens_success() {
        let mut staking = StakingContract::new();
        staking.stake_tokens("user1".to_string(), 1000.0, 30).unwrap();
        
        let result = staking.unstake_tokens("user1");
        assert!(result.is_ok());
        let (amount, rewards) = result.unwrap();
    ...
    fn test_emergency_withdrawal_disabled() {
        ...
        let result = staking.emergency_withdraw("user1");
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), StakingError::EmergencyWithdrawalsDisabled);
    }
    ...
    fn test_transfer_staking_position_exists() {
        let mut staking = StakingContract::new();
        staking.stake_tokens("user1".to_string(), 1000.0, 30).unwrap();
        staking.stake_tokens("user2".to_string(), 500.0, 30).unwrap();
