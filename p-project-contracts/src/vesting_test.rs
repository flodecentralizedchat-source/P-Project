...
fn test_founding_member_vesting_schedule() {
    ...
    let result = vesting_contract.create_founding_member_vesting(user_id.clone(), amount, start_date);
    assert!(result.is_ok());
...
fn test_multiple_founding_members_vesting() {
    ...
    for i in 1..=13 {
        ...
        let result = vesting_contract.create_founding_member_vesting(user_id, amount_per_member, start_date);
        assert!(result.is_ok());
    }
    ...
    assert_eq!(vesting_contract.get_total_vesting_tokens(), total_vesting_tokens);
}
...
fn test_founder_and_founding_member_vesting_together() {
    ...
    let result = vesting_contract.create_founder_vesting(founder_id.clone(), founder_amount, start_date);
    assert!(result.is_ok());
    
    // Create vesting for Founding Member 1
    let member_id = "founding_member_1".to_string();
    let member_amount = 2692308.0;
    let result = vesting_contract.create_founding_member_vesting(member_id.clone(), member_amount, start_date);
    assert!(result.is_ok());
