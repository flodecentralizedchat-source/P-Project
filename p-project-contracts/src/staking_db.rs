...
impl StakingDbAdapter {
    ...
    pub async fn save_staking_info(&self, user_id: &str, staking_info: &StakingInfo) -> Result<(), StakingError> {
        let staking_json = serde_json::to_string(staking_info)
            .map_err(|e| StakingError::SerializationError(format!("Failed to serialize staking info: {}", e)))?;
    ...
    pub async fn load_staking_info(&self, user_id: &str) -> Result<Option<StakingInfo>, StakingError> {
        let row = sqlx::query("SELECT staking_data FROM staking_infos WHERE user_id = ?")
            .bind(user_id)
            .fetch_optional(self.mysql.as_ref().get_pool())
            .await
            .map_err(|e| StakingError::DatabaseError(format!("Failed to load staking info: {}", e)))?;

        if let Some(row) = row {
            let staking_json: String = row.get("staking_data");
            let staking_info: StakingInfo = serde_json::from_str(&staking_json)
                .map_err(|e| StakingError::SerializationError(format!("Failed to deserialize staking info: {}", e)))?;
