...
fn test_complete_team_allocation_protection() {
    ...
    let founder1_result =
        vesting_contract.create_founder_vesting("founder_1".to_string(), 35000000.0, start_date);
    ...
    let founder2_result =
        vesting_contract.create_founder_vesting("founder_2".to_string(), 35000000.0, start_date);
    ...
    for i in 1..=13 {
        ...
        let member_result =
            vesting_contract.create_founding_member_vesting(member_id, 2692308.0, start_date);
        assert!(member_result.is_ok());
    }

    // Verify one founding member schedule
    let member_schedule = vesting_contract
        .get_vesting_schedule("founding_member_1")
        .unwrap();
...
fn test_anti_dump_protection_mechanisms() {
    ...
    let claimable = vesting_contract
        .calculate_claimable_amount("founder_1")
        .unwrap();
    ...
    // Check that no tokens are claimable during cliff period (anti-dump protection)
    let claimable = vesting_contract
        .calculate_claimable_amount("member_1")
        .unwrap();
    assert_eq!(claimable, 0.0);
...
fn test_vesting_contract_immutable_properties() {
    ...
    vesting_contract
        .create_founder_vesting("founder_1".to_string(), 35000000.0, start_date)
        .unwrap();
