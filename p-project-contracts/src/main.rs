use p_project_contracts::{AirdropContract, PProjectToken, StakingContract};
...
fn main() {
    ...
    println!(
        "Token contract created with total supply: {}",
        token_contract.get_total_supply()
    );
    ...
    match staking_contract.stake_tokens("user1".to_string(), 50000.0, 30) {
        Ok(tier) => println!("Staking successful with {} tier", tier),
