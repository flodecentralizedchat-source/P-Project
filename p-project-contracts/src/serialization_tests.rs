...
mod tests {
    ...
    use p_project_core::models::{
        StakingInfo as CoreStakingInfo, TokenTransaction, TransactionType,
    };
    use serde_json;
    ...
    fn test_token_serialization() {
        ...
        let deserialized_token: PProjectToken =
            serde_json::from_str(&json).expect("Failed to deserialize token");

        // Verify the deserialized token has the same properties
        assert_eq!(
            deserialized_token.get_total_supply(),
            token.get_total_supply()
        );
        assert_eq!(
            deserialized_token.get_balance("user1"),
            token.get_balance("user1")
        );
        assert_eq!(
            deserialized_token.get_balance("user2"),
            token.get_balance("user2")
        );
    }
    ...
    fn test_token_event_serialization() {
        ...
        // Deserialize from JSON
        let deserialized_event: TokenEvent =
            serde_json::from_str(&json).expect("Failed to deserialize token event");
    ...
    fn test_token_transaction_serialization() {
        ...
        // Serialize to JSON
        let json =
            serde_json::to_string(&transaction).expect("Failed to serialize token transaction");
        assert!(!json.is_empty());

        // Deserialize from JSON
        let deserialized_transaction: TokenTransaction =
            serde_json::from_str(&json).expect("Failed to deserialize token transaction");

        // Verify the deserialized transaction has the same properties
        assert_eq!(deserialized_transaction.id, transaction.id);
        assert_eq!(
            deserialized_transaction.from_user_id,
    ...
    fn test_staking_contract_serialization() {
        ...
        // Deserialize from JSON
        let deserialized_staking: StakingContract =
            serde_json::from_str(&json).expect("Failed to deserialize staking contract");

        // Verify the deserialized staking contract has the same properties
        assert_eq!(
            deserialized_staking.get_staking_tiers().len(),
            staking.get_staking_tiers().len()
        );
    }
    ...
    fn test_core_staking_info_serialization() {
        ...
        // Serialize to JSON
        let json =
            serde_json::to_string(&staking_info).expect("Failed to serialize core staking info");
        assert!(!json.is_empty());

        // Deserialize from JSON
        let deserialized_staking_info: CoreStakingInfo =
            serde_json::from_str(&json).expect("Failed to deserialize core staking info");
        ...
        assert_eq!(
            deserialized_staking_info.rewards_earned,
            staking_info.rewards_earned
        );
        assert_eq!(deserialized_staking_info.tier_name, staking_info.tier_name);
        assert_eq!(
            deserialized_staking_info.is_compounding,
            staking_info.is_compounding
        );
    }
    ...
    fn test_airdrop_contract_serialization() {
        ...
        // Deserialize from JSON
        let deserialized_airdrop: AirdropContract =
            serde_json::from_str(&json).expect("Failed to deserialize airdrop contract");
    ...
    fn test_complex_nested_serialization() {
        ...
        let deserialized_token: PProjectToken =
            serde_json::from_str(&json).expect("Failed to deserialize complex token");
        assert_eq!(
            deserialized_token.get_total_supply(),
            token.get_total_supply()
        );
        assert_eq!(
            deserialized_token.get_transaction_log().len(),
            token.get_transaction_log().len()
        );
        assert_eq!(
            deserialized_token.get_event_log().len(),
            token.get_event_log().len()
        );
    }
}
