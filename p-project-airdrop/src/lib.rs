...
impl AirdropService {
    ...
    pub async fn add_recipients_with_category(&mut self, recipients: Vec<(String, f64)>, category: Option<String>) -> Result<(), String> {
        // Add to contract first
        let result = if let Some(ref cat) = category {
            self.airdrop_contract.add_recipients_with_category(recipients.clone(), Some(cat.clone()))
        } else {
        ...
        if result.is_ok() {
            ...
            self.db.add_airdrop_recipients(airdrop_id, &recipients, category_str)
                .await
    ...
    pub async fn claim_airdrop(&mut self, user_id: &str) -> Result<f64, String> {
        ...
        if result.is_ok() {
            // Save to database
            let airdrop_id = self.airdrop_contract.get_airdrop_id();
            self.db.claim_airdrop(airdrop_id, user_id)
                .await
    ...
    pub async fn batch_claim_airdrops(&mut self, user_ids: Vec<String>) -> Result<Vec<(String, f64)>, String> {
        ...
        if result.is_ok() {
            // Save to database
            let airdrop_id = self.airdrop_contract.get_airdrop_id();
            self.db.batch_claim_airdrops(airdrop_id, &user_ids)
                .await
    ...
    pub async fn is_claimed(&self, user_id: &str) -> Result<bool, String> {
        // Check in database
        let airdrop_id = self.airdrop_contract.get_airdrop_id();
        self.db.is_airdrop_claimed(airdrop_id, user_id)
            .await
    ...
    pub async fn get_status(&self) -> Result<p_project_contracts::airdrop::AirdropStatus, String> {
        // Get from database
        let airdrop_id = self.airdrop_contract.get_airdrop_id();
        let (total_amount, distributed_amount, total_recipients, claimed_recipients) = 
            self.db.get_airdrop_status(airdrop_id)
                .await
